...
class ReplayBuffer:
    def __init__(self, capacity, device, n_step=1, gamma=0.99):
        ...
        self.n_step = n_step
        self.gamma = gamma
        self.n_step_buffer = []  # Temporary buffer for n-step transitions
    ...
    def append(self, s, a, r, s_, d):
        ...
        if len(self.n_step_buffer) == self.n_step:
            R, s_n, done_n = self._calc_n_step_return()
            s_0, a_0, _, _, _ = self.n_step_buffer[0]
            self._add_transition(s_0, a_0, R, s_n, done_n)
            self.n_step_buffer.pop(0)  # Remove oldest transition

        # If terminal state, flush the buffer
        if d:
    ...
    def _calc_n_step_return(self):
        ...
        R, s_n, done_n = 0, self.n_step_buffer[-1][3], self.n_step_buffer[-1][4]
        for i, (_, _, r, _, _) in enumerate(self.n_step_buffer):
            R += (self.gamma ** i) * r
        return R, s_n, done_n

    def _add_transition(self, s, a, R, s_, d):
...
class ProjectAgent:
    ...
    def gradient_step(self):
        if len(self.memory) > self.config['batch_size']:
            ...
            with torch.no_grad():
                ...
                target = R + (1 - D) * (self.config['gamma'] ** self.config['n_step']) * next_q_values
    ...
    def act(self, state):
        ...
        with torch.no_grad():
            ...
            if not self.training and np.sum(self.action_count)==199:
                action = 3
        ...
        with tqdm(total=max_episodes, desc="Training Progress", unit="episode") as pbar:
            while len(episode_returns) < max_episodes:
                ...
                if in_exploration:
                    ...
                    if len(episode_returns) < max_episodes/2:
                        epsilon= 1-2*len(episode_returns)/max_episodes
                   # elif  len(episode_returns) < max_episodes-400:
                     #   epsilon= 1e-1
                    else : 
                        epsilon=0
                ...
                if done or truncated:
                    ...
                    action_probs = self.action_count / total_actions if total_actions > 0 else self.action_count
                    ...
                    if len(episode_returns)%100 ==0:
                        save_path = f"Logs/NoisyNet/saved_model3_{len(episode_returns)}"
                        save_path_returns = f"Logs/NoisyNet/saved_returns3"
                        np.save(save_path_returns, np.array(episode_returns))
                        self.save(save_path)
                else:
                    state = next_state
...
if __name__ == "__main__":
    ...
    agent = ProjectAgent(N_explo=50, T_explo=20)
    agent.training = True
